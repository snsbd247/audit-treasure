...
const BackupPage = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const blob = new Blob([result.content], {
        type: exportFormat === "sql" ? "text/sql" : "application/json",
      });
      ...
      toast({
        ...
        description: `${result.tables_count} tables, ${result.records_count} records exported as ${exportFormat.toUpperCase()}.`,
      });
      fetchHistory();
    } catch (err: any) {
  ...
  const executeRestore = async () => {
    ...
    try {
      ...
      if (format === "sql") {
        toast({
          title: "SQL Restore",
          description: "SQL backups must be restored via the Supabase SQL Editor. Please paste the SQL content there.",
          variant: "destructive",
        });
        setRestoring(false);
        setProgress(0);
      ...
      try {
        ...
        throw new Error("Invalid JSON file — could not parse backup content.");
      }
      ...
      toast({
        ...
        description: `${result.records_restored} records restored.${result.errors?.length ? ` ${result.errors.length} warnings.` : ""}`,
      });
      fetchHistory();
    } catch (err: any) {
  ...
  const handleDownloadFromHistory = async (record: BackupRecord) => {
    if (!record.storage_path) {
      toast({ title: "File not available", description: "No stored file for this backup.", variant: "destructive" });
      return;
    }
    try {
  ...
  const handleDeleteBackup = async (id: string) => {
    ...
    await supabase.from("backup_history" as any).delete().eq("id", id);
  ...
  const saveSettings = async (updates: Partial<BackupSettings>) => {
    ...
    await supabase.from("backup_settings" as any).update(newSettings as any).eq("id", "default");
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      case "completed":
        return <Badge variant="default" className="bg-green-600 text-white"><CheckCircle className="w-3 h-3 mr-1" />Completed</Badge>;
      case "failed":
        return <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1" />Failed</Badge>;
      case "completed_with_errors":
        return <Badge variant="secondary" className="bg-yellow-600 text-white"><AlertTriangle className="w-3 h-3 mr-1" />Partial</Badge>;
      default:
        return <Badge variant="outline"><RefreshCw className="w-3 h-3 mr-1 animate-spin" />In Progress</Badge>;
    }
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              {exportFormat === "json"
                ? "JSON format — portable, supports programmatic restore via this page."
                : "SQL format — INSERT statements with ON CONFLICT. Restore via Supabase SQL Editor."}
            </p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-muted-foreground">
              Upload a .json backup file. Records are upserted — existing records are updated, new ones added. SQL files must be run in Supabase SQL Editor.
            </p>
            <label>
            ...
            <div className="flex flex-wrap gap-6 items-end">
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  onCheckedChange={(v) => saveSettings({ auto_backup_enabled: v })}
              ...
              {settings.last_auto_backup_at && (
                <p className="text-xs text-muted-foreground">
                  Last auto backup: {formatDate(settings.last_auto_backup_at)}
                </p>
              )}
            </div>
          </CardContent>
                ...
                <TableBody>
                  {history.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>
                        <Badge variant="outline" className="text-xs capitalize">{r.backup_type}</Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant="secondary" className="text-xs uppercase">{r.format}</Badge>
                      </TableCell>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2"><FileText className="w-4 h-4" />Backup Information</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-sm text-muted-foreground space-y-2">
            ...
            <p>• <strong>Security:</strong> Only Super Admins can create backups and restore data</p>
            <p>• <strong>Storage:</strong> Backup files are stored in secure Supabase Storage</p>
            <p>• <strong>Audit:</strong> All backup and restore operations are logged</p>
          </div>
        </CardContent>
      </Card>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              This will update existing records and add new ones. This action cannot be easily undone.
            </DialogDescription>
          </DialogHeader>
          <div className="bg-destructive/10 border border-destructive/20 rounded p-3 text-sm text-destructive">
            <strong>Warning:</strong> It is recommended to create a backup before restoring to prevent data loss.
          </div>
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={executeRestore}>
              <Upload className="w-4 h-4 mr-2" />Confirm Restore
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!confirmDelete} onOpenChange={() => setConfirmDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete this backup record and its stored file. Continue?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
