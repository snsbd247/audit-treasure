    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between px-6 py-3 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-base font-medium text-foreground tracking-tight">Vouchers</h1>
        </div>
        <motion.button
          whileTap={{ scale: 0.97 }}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-80 border-r border-border overflow-hidden flex flex-col">
          <VoucherList vouchers={vouchers} selectedId={selectedId} onSelect={setSelectedId} />
        </div>
        <div className="flex-1 overflow-hidden">
          {selectedVoucher ? (
            <VoucherDetail voucher={selectedVoucher} onApprove={handleApprove} onReject={handleReject} onSubmit={handleSubmit} />
          ) : (
