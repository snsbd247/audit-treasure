...
export default function MyLeavePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [empRes, ltRes] = await Promise.all([
        supabase.from("employees" as any).select("id").eq("user_id", user.id).single(),
  ...
  useEffect(() => {
    if (!employee) return;
    (async () => {
      const { data } = await supabase.from("leave_requests" as any).select("*").eq("employee_id", (employee as any).id).order("created_at", { ascending: false });
      if (data) setRequests(data as any);
    })();
  }, [employee]);

  const handleSubmit = async () => {
    if (!form.leave_type_id || !form.start_date || !form.end_date) { toast.error("All fields required"); return; }
    const { error } = await supabase.from("leave_requests" as any).insert({
      employee_id: (employee as any).id, leave_type_id: form.leave_type_id,
